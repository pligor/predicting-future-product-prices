...
from mylibs.batch_norm import BatchNormer, batchNormWrapper
...
class PriceHistorySeq2SeqDynDecIns(CrossValidator):
    FEATURE_LEN = 1
    ...
    def __init__(self, rng, dtype, config, with_EOS=False):
        super(PriceHistorySeq2SeqDynDecIns, self).__init__(random_state=rng,
    ...
    class RNN_CELLS(object):
        DEFAULT_ACTIVATION = tf.nn.tanh  # tf.nn.elu
        ...
        @staticmethod
        def GRU(activation=DEFAULT_ACTIVATION):
    ...
    def get_cross_validation_score(self, npz_path, epochs, batch_size, input_len, target_len, n_splits,
                                   ...
                                   activation=RNN_CELLS.DEFAULT_ACTIVATION,
        ...
        graph = self.getGraph(batch_size=batch_size, verbose=False, num_units=num_units, input_len=input_len,
                              rnn_cell=rnn_cell(activation), target_len=target_len, cost_func=cost_func,
                              eos_token=eos_token,
        ...
        for _, preds, targets in tuples:
            ...
            for cur_key in preds:
                # print type(targets)
                # print type(targets[cur_key])
                # print targets.shape
                # print type(preds)
                # print type(preds[cur_key])
                # print preds.shape
                # temp = fastdtw(targets[cur_key], preds[cur_key])
                # print type(temp)
                # print temp
    ...
    def run(self, npz_path, npz_test, epochs, batch_size, num_units, input_len, target_len,
            decoder_first_input=DECODER_FIRST_INPUT.ZEROS,
            activation=RNN_CELLS.DEFAULT_ACTIVATION,
            batch_norm_enabled=BATCH_NORM_ENABLED_BY_DEFAULT,
            lamda2=DEFAULT_LAMDA2,
        ...
        preds_dp = PriceHistorySeq2SeqDataProvider(npz_path=npz_test, batch_size=batch_size, rng=self.rng,
    ...
    def getGraph(self,
        ...
        with graph.as_default():
            ...
            #
            #     # note that we are using exactly the same that we used above inside RNN cell (not sure if this is the best)
    ...
    def pass_decoder_out_through_vars(self, layer, variables, verbose):
        ...
        for ii in range(start, stop, step):
            ...
            activation_fn = tf.identity if ii + step >= stop else tf.nn.elu
            layer = activation_fn(tf.add(tf.matmul(layer, cur_WW), cur_bb), "dense_hidden_{}".format(ii / 2))

        # cur_WW = variables[-2]
        # cur_bb = variables[-1]
        # affine_layer = tf.add(tf.matmul(layer, cur_WW), cur_bb)
        return layer
