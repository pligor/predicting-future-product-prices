...
class MyBaseline(object):
    ...
    def renderRandomTargetVsPrediction(self):
        ...
        plt.plot(range(0, 60), self.inputs[ind, :, self.INDEX_PRICE_INPUT].flatten(), 'r')
    ...
    def renderRandomMultipleTargetsVsPredictions(self):
        # fig = plt.figure(figsize=(15, 7))
        ...
        for ax in axes.flatten():
            ind = np.random.randint(len(self.targets))
            ax.plot(range(0, 60), self.inputs[ind, :, self.INDEX_PRICE_INPUT].flatten(), 'r', label='past')
            ax.plot(range(60, 90), self.targets[ind].flatten(), 'b', label='real future')
            ax.plot(range(60, 90), self.preds[ind].flatten(), 'g', label='predicted future')
            plt.legend(loc='best')
            # plt.show()
