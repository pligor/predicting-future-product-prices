...
from models import ADAM_DEFAULT_LEARNING_RATE

class PriceHistorySeq2SeqNativeBatchNorm(CrossValidator):
    FEATURE_LEN = 1
    TARGET_FEATURE_LEN = 1

    def __init__(self, rng, dtype, config, debug=False, with_EOS=False):
        # super(ShiftFunc, self).__init__(random_state=rng, data_provider_class=ProductPairsDataProvider)
        super(PriceHistorySeq2SeqNativeBatchNorm, self).__init__(random_state=rng,
    ...
    def get_cross_validation_score(self, npz_path, epochs, batch_size, num_units, input_len, target_len, n_splits,
        ...
        # TODO this choice of minimum validation error could change to the last or the average of the last few
        ...
        with graph.as_default():
            ...
            with tf.name_scope('encoder_rnn_layer'):
                ...
                _, encoder_final_state = rnn.static_rnn(cell=rnn_cell(num_units=num_units),
                                                        ...
                                                        initial_state=None,
                                                        # TODO when using trunc backprop this should not be None (zero)
                                                        dtype=self.dtype)
