...
# DYNAMIC SEQUENCES - HEAVY MODEL - PLUS DIFF OPTIMIZER
class PriceHistoryAutoencoder(NeuralNetModelInterface):
    ...
    *) Make also input be depende on previous input ??
    ...
    def run(self, npz_path, epochs, batch_size, enc_num_units, dec_num_units, ts_len,
            ...
            learning_rate_diff=ADAM_DEFAULT_LEARNING_RATE,
        ...
        graph = self.getGraph(batch_size=batch_size, verbose=False, enc_num_units=enc_num_units,
                              ...
                              learning_rate=learning_rate, learning_rate_diff=learning_rate_diff,
                              hidden_enc_num_units=hidden_enc_num_units,
        ...
        self.__print_hyperparams(learning_rate=learning_rate, learning_rate_diff=learning_rate_diff,
                                 epochs=epochs, enc_num_units=enc_num_units, dec_num_units=dec_num_units)
    ...
    def train_validate(self, train_data, valid_data, **kwargs):
        ...
        with tf.Session(graph=graph, config=self.config) as sess:
            ...
            dyn_stats = DynStats(validation=valid_data is not None)
            ...
            for epoch in range(epochs):
                ...
                if valid_data is None:
                    ...
                    if verbose:
                        print 'EndEpoch%02d(%.3fs):err(tr)=%.6f,errdiff(tr)=%.6f,err(val)=%.6f,errdiff(val)=%.6f' % (
                            epoch + 1, runTime, train_error, train_error_diff, valid_error, valid_error_diff)

                    dyn_stats.gatherStats(train_error=train_error, valid_error=valid_error)
    ...
    def getGraph(self,
        ...
        with graph.as_default():
            ...
            with tf.name_scope('error_diff'):
                ...
                seq_mask_without_first_elem = tf.cast(sequence_len_mask[:, 1:], tf.float32)
                lossed_diff = huber_loss(y_true=targets_diff, y_pred=outputs_diff) * seq_mask_without_first_elem

                if verbose:
                    print lossed_diff
                    print

                loss_diff = tf.reduce_mean(lossed_diff)
            ...
            with tf.name_scope('training_step'):
                ...
                train_step_diff = tf.train.AdamOptimizer(learning_rate=learning_rate_diff).minimize(loss_diff)
