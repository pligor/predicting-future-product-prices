...
class PriceHistory27DatasetGenerator(PriceHistoryTrait):
    """
    ...
    def __init__(self, random_state):
        super(PriceHistory27DatasetGenerator, self).__init__()
    ...
    def generate_dataset(self, df, input_seq_len, target_seq_len,
        ...
        train_pack = PriceHistory27Pack(input_seq_len=input_seq_len, target_seq_len=target_seq_len)
        test_pack = PriceHistory27Pack(input_seq_len=input_seq_len, target_seq_len=target_seq_len)

        for is_training, sku_id, inputs, date_inputs, targets, date_targets in self.getInputsTargets(
            ...
            if is_training:
                train_pack.update(sku_id=sku_id, inputs=inputs, targets=targets, input_seq_len=input_seq_len,
                                  date_inputs=date_inputs, date_targets=date_targets)
            else:
    ...
    def processSeq(self, seq, is_training, sku_id, window_len, target_len, allowSmallerSequencesThanWindow,
        ...
        for seq_window in self.getSeqWindows(
            ...
            if seq_window is None:
                ...
                # print seq_window.index
                # exit(16)
                date_inputs = seq_window.index[:-target_len]
                inputs_flat = seq_norm[:-target_len]

                # inputs = inputs_flat[np.newaxis].T

                date_targets = seq_window.index[-target_len:]
                targets = seq_norm[-target_len:]

                if normalize_targets:
                    last_input = inputs_flat[-1]
                    processed_targets = self.removeBiasFromSeq(targets, subtract=last_input)
                else:
                    processed_targets = targets

                # final_targets = processed_targets[np.newaxis].T

                yield is_training, sku_id, inputs_flat, date_inputs, processed_targets, date_targets
    ...
    def merge_date_info(npz_path):
        ...
        necessary_keys = {'inputs', 'targets', 'date_inputs', 'date_targets'}
        assert len(
            necessary_keys.difference(set(dic.keys()))) == 0, "npz should contain at least these keys: {}".format(
            necessary_keys)

        XX = []
        for cur_input, cur_date_stream in zip(dic['inputs'], dic['date_inputs']):
            date_info = np.array([convert_date_str_to_info(cur_date).values() for cur_date in cur_date_stream])
        ...
        for cur_input, cur_date_stream in zip(dic['targets'], dic['date_targets']):
            date_info = np.array([convert_date_str_to_info(cur_date).values() for cur_date in cur_date_stream])
            decoder_extra_inputs.append(date_info)

            shaped_in = cur_input[np.newaxis].T
            full_info = np.concatenate((shaped_in, date_info), axis=1)
            decoder_inputs.append(full_info)
        ...
        del copydic['date_targets']  # erase merged info
        copydic['decoder_inputs'] = np.array(decoder_inputs)
        copydic['decoder_extra_inputs'] = np.array(decoder_extra_inputs)
