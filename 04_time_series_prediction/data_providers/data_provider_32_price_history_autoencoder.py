...
class PriceHistoryAutoEncDataProvider(CrossValDataProvider):  # , SubSampleableDataProvider):
    ...
    def __init__(self,
        ...
        trim_slice = slice(0, dic[dic.keys()[0]].shape[1]) if ts_max_len is None else slice(0, ts_max_len)
        inputs, decoder_extra_inputs = dic['inputs'][:, trim_slice], dic['extra_inputs'][:, trim_slice]
        ...
        super(PriceHistoryAutoEncDataProvider, self).__init__(
            datalist=[inputs, decoder_extra_inputs],
    ...
    def next(self):
        ...
        """
        inputs_batch, decoder_extra_inputs_batch = super(PriceHistoryAutoEncDataProvider, self).next()
        ...
        return inputs_batch, final_dec_extra_ins
