...
/**
 * An abstract canvas containg a single {@link Plot}.
 * The canvas is specified by a {@link ClippingRectangle}, called 
 * <em>paper</em>. A horizontal and vertical {@link Anchor} determine
 * the position of the paper on the actual device.
 * 
...
public class PlotCanvas implements PlotListener {
   ...
   * <tr><td><tt>horizontalAnchor = center</tt></td>
   *     <td><tt>String</tt></td><td>no</td>
   *     <td>Horizontal position of the paper relative to the device
   *         border. Possible values are <tt>left</tt>, <tt>center</tt>,
   *         and <tt>right</tt>.</td></tr>
   * <tr><td><tt>paper = 0,&nbsp;0,&nbsp;1,&nbsp;0.6</tt></td>
   *     <td><tt>double[]</tt></td><td>no</td>
   *     <td>Rectangle defining the paper. The first two values determine
   *         the x- and y- coordinates (in device-independent units) 
   *         of the lower-left corner. The last two values determine the 
   *         upper-right corner.</td></tr>
   * <tr><td><tt>plot = </tt>default values of {@link Plot}</td>
   *     <td><tt>ConfigParameters</tt></td><td>no</td>
   *     <td>Definition of the {@link Plot}.</td></tr>
   * <tr><td><tt>verticalAnchor = center</tt></td>
   *     <td><tt>String</tt></td><td>no</td>
   *     <td>Vertical position of the paper relative to the device
   *         border. Possible values are <tt>top</tt>, <tt>center</tt>,
   *         and <tt>bottom</tt>.</td></tr>
   * </table>
   * <p>
   * Note, that this instance registers itself at the  
   * wrapped {@link Plot} instance.
   */
  public PlotCanvas(ConfigParameters config) {
    double[] paper = config.getDoubleArray(PAPER_KEY,
                                           new double[] {0, 0, 1, 0.6});
    _paper = new ClippingRectangle(paper[0], paper[1], paper[2], paper[3]);
    _horizontalAnchor = Anchor.getHorizontalAnchor(config,
                                  HORIZONTAL_ANCHOR_KEY, Anchor.CENTER);
   ...
   * {@link DataPlot}.
   * @param dataPlot Data to be connected with this plot canvas.
  ...
  /**
   * Handles the spcified event. Here nothing is done. But
